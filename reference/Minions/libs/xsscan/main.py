...
import libs.xsscan.fuzzing
import libs.xsscan.fuzzing.vectors
import libs.xsscan.fuzzing.DCP
import libs.xsscan.fuzzing.DOM
import libs.xsscan.fuzzing.HTTPsr
import libs.xsscan.fuzzing.heuristic
from collections import defaultdict
from itertools import islice, chain
from libs.xsscan.curlcontrol import Curl
from libs.xsscan.encdec import EncoderDecoder
from libs.xsscan.options import XSSerOptions
from libs.xsscan.dork import Dorker
from libs.xsscan.crawler import Crawler
from libs.xsscan.post.shorter import ShortURLReservations
from libs.xsscan.imagexss import ImageInjections
from libs.xsscan.flashxss import FlashInjections
from libs.xsscan.publish import publisher
from libs.xsscan.post.xml_exporter import xml_reporting
from libs.xsscan.tokenhub import HubThread
from libs.xsscan.reporter import XSSerReporter
from libs.xsscan.threadpool import ThreadPool, NoResultsPending
...
class xsser(EncoderDecoder, XSSerReporter):
    ...
    def __init__(self, mothership=None):
        self.heuris_test = ''                          #保存启发式扫描结果，探测未过滤字符
        self.successfull_payloads = ''                 #保存成功的payloads
        self.taskid = self.generate_hash(attack_type='taskid')        #生成唯一任务id，用于数据库保存
    ...
    def generate_taskid(self,attack_type='taskid'):
        return hashlib.md5(str(datetime.datetime.now()) + attack_type).digest()
    ...
    def get_payloads(self):
        ...
        options = self.options
        # payloading sources
        payloads_fuzz = libs.xsscan.fuzzing.vectors.vectors
        payloads_dcp = libs.xsscan.fuzzing.DCP.DCPvectors
        payloads_dom = libs.xsscan.fuzzing.DOM.DOMvectors
        payloads_httpsr = libs.xsscan.fuzzing.HTTPsr.HTTPrs_vectors
        ...
        heuristic_params = libs.xsscan.fuzzing.heuristic.heuristic_test
    ...
    def add_success(self, dest_url, payload, hashing, query_string, orig_url, method='url'):
        ...
        if payload['browser'] == "[manual_injection]":
            ...
            if self.heuris_test != '':
                self.heuris_test += ',' + str(payload['payload']).strip('XSS')
            else:
                self.heuris_test += str(payload['payload']).strip('XSS')            
        else:
            self.report("[+] Checking: " + method + " attack with " + payload['payload'].strip() + "... ok\n")
    ...
    def create_gtk_interface(self):
        ...
        """
        options = self.options
        from xsscan.gtkcontroller import Controller, reactor
        uifile = "xsser.ui"
    ...
    def save_result(self,line):
        ...
        import time
        if line[5]["browser"] != "[Heuristic test]":
            target = line[6]
            poc = line[0].replace("'", "''")
            browsers = line[1]
            heuristic = self.heuris_test.replace("'", "''")
            t = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())
            db = database()
            cur = db.connectdb('./db.sqlite3')
            sqlcmd = """insert into minions_xsscan(taskid,target,heuristic,poc,browsers,time)values('%s','%s','%s','%s','%s','%s')""" % (self.taskid,target,heuristic,poc,browsers,t)
