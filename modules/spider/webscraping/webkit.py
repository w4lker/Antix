...
from PyQt4.QtCore import QByteArray, QString, QUrl, QTimer, QEventLoop, QIODevice, QObject, QVariant,pyqtSignal
...
class NetworkAccessManager(QNetworkAccessManager):
    ...
    postSignal = pyqtSignal()      
    def createRequest(self, operation, request, data):
        url = request.url().toString().toUtf8().data()
        print url
        self.req_urls.append(url)

        
        if operation == self.PostOperation and data.read(1024) != None:
...
class WebkitBrowser(QWebView):
    ...
    def request(self):
        print self.page().networkAccessManager().req_urls
    ...
    def linkClick(self,url):
        print url.toString().toUtf8().data()
...
if __name__ == '__main__':
    ...
    # load webpage
    w.get('http://news.qq.com/')

    # fill search textbox 
    w.fill('input[type=text]', 'sitescraper')
    w.js('document.location.host=gov.cn')
    print 'click'
