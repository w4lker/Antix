...
from PyQt5.QtCore import QUrl,QSize,pyqtSignal,QThread,QEventLoop,QThreadPool,Qt,QTimer
...
class WebkitBrowser(QWebView):
    ...
    def linkClick(self,url):
        self.manager.req_urls.append(url.toString())
        print url.toString()

if __name__=='__main__':
    start = time()
    r = WebkitBrowser(gui=True,)
    r.get('http://demo.aisec.cn/demo/aisec/')
    r.attr('form','enctype','')
    print r.find('form')
    #r.click(pattern='a')
    r.click(pattern='input')
    print r.manager.req_urls
